...
CILIN_PATH = Path(__file__).parent.joinpath('data/cilin.txt')
CONFUSION_PATH = Path(__file__).parent.joinpath('data/confusion_dict.txt')
chinese_punct = "！？｡＂＃＄％＆＇（）＊＋，－／：；＜＝＞＠［＼］＾＿｀｛｜｝～｟｠｢｣､、〃》「」『』【】〔〕〖〗〘〙〚〛〜〝〞〟〰〾〿–—‘'‛“”„‟…‧﹏"
...
def read_cilin():
    ...
    for line in lines:
        ...
        for word in words:
            semantic_dict[word] = code  # word重复的时候怎么处理？例如'人'在词林txt里面出现了5次，不同code line
        # make reverse dict
...
class Alignment:
    ...
    def __init__(
        ...
        """
        构造函数
        :param semantic_dict: 语义词典（大词林）同义词词典
        ...
        # POS tag is not utilized in character mode tokenization.
    ...
    def __call__(self,
        ...
        if verbose:
            ...
            print(align_seq)
            print("========== Results ==========")
            for aseq in align_seq:  # (index Error, modified by pe653 on 221204)
                for a in aseq:
        ...
        for i in range(len(src)):
            for j in range(len(tgt)):
                ...
                # Non-matches
                    ...
                    sub_cost = cost_matrix[i][j] + self.get_sub_cost(src[i], tgt[j])  # 由替换动作得到的总cost
